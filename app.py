...
CFG = {
    ...
    #-------------------------------------------------
    # 2차 게이트: 31 / 32 한 묶음 횡보장 / 미세 흔들림 차단 
    "31_EMA_SLOPE_LOOKBACK_BARS": 3,  
    ## 의미: EMA 기울기를 계산할 과거 봉 개수
    #  - 5분봉 기준 → 5 = 최근 25분 방향성
    # 추천값:
    #  - 기본값: 5
    #  - 추천범위: 3 ~ 7
    # 사유:
    #  - 3 이하  → 노이즈에도 기울기 발생 (횡보장 통과 위험)
    #  - 5       → 방향성/반응속도 균형점 (권고)
    #  - 7 이상  → 추세 확인은 되나 진입이 늦어짐
    "32_EMA_SLOPE_MIN_PCT":    0.02,
    # 의미:
    #  - EMA 기울기의 최소 퍼센트 기준
    #  - 이 값 이상 기울어져야 "의미 있는 추세"로 판단
    # 추천값:
    #  - 기본값: 0.03
    #  - 추천범위: 0.02 ~ 0.05
    # 사유:
    #  - 0.02 → 횡보장 미세 꺾임도 통과 (가짜 추세 많음)
    #  - 0.03 → 수수료·슬리피지 제거 후 실추세 경계 (권고)
    #  - 0.05 → 강추세만 통과, 체결 급감    
    ...
    # [34] EXECUTION_MIN_PRICE_MOVE_PCT
    # 의미:
    #  - 후보(Candidate) 가격 대비
    #    실제 가격이 이 퍼센트 이상 움직였을 때만 체결 허용
    # 추천값:
    #  - 기본값: 0.15
    #  - 추천범위: 0.10 ~ 0.20
    # 사유:
    #  - 0.10 → 체결 빈도는 높으나 틱노이즈 체결 다수 발생
    #  - 0.15 → 노이즈 제거 + 추세 초입 포착의 균형점 (권고)
    #  - 0.20 → 강한 움직임만 진입, 체결 급감 / 안정성 ↑
    # 체감 조정 가이드:
    #  - 체결이 너무 많고 수익이 얇다 → 0.18 ~ 0.20
    #  - 체결이 거의 없다            → 0.12 ~ 0.14
